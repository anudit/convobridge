...
export const getBridgeData = async (ethAddress) =>{
    ...
    if (snapshot.length > 0){
        return {
            ...
            twitch: Object.keys(snapshot[0]).includes('twitchData') === true ? (snapshot[0].twitchData?.name) : false
        };
    }
    else {
        return { success: true };
...
export const bridgeReverseLookup = async (type, userId) =>{
    ...
    if ( type === 'telegram' ) {
        ...
        query = new Where("twitchData.name").eq(userId);
    }
    else {
...
export const updateAuthData  = async (type, ethAddress, data) =>{
    ...
    if (['telegram','slack','discord','zoom','spotify','twitch'].includes(type)) {
        let key = type+"Data";
        await threadClient.save(threadId, 'bridge', [{
            ...bridgeData,
            [key]: data,
